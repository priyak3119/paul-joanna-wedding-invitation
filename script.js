...
document
  ...
  .addEventListener("click", () => {
    ...
    const revealAfter = reducedMotion ? 0 : 1100;
    const fadeDuration = reducedMotion ? 180 : 650;
    entry.style.setProperty("--entry-fade-duration", `${fadeDuration}ms`);
    entry.classList.add("opening");
    if (!reducedMotion) {
      const dust = entry.querySelector(".opening-dust");
      for (let i = 0; i < 56; i++) {
        const fleck = document.createElement("i");
        fleck.style.setProperty("--dx", ((i * 73 % 340) - 170) + "px");
        fleck.style.setProperty("--dy", (-90 - (i * 47 % 370)) + "px");
        fleck.style.setProperty("--delay", (0.25 + (i % 9) * 0.055) + "s");
...
function initScratch() {
  ...
  gradient.addColorStop(0, "#8b7247");
  gradient.addColorStop(0.36, "#dfcca0");
  gradient.addColorStop(0.66, "#b69b65");
  gradient.addColorStop(1, "#78613d");
  ...
  sheen.addColorStop(0, "#fff9e8a0");
  sheen.addColorStop(0.52, "#f4e8cd3d");
  ...
  // Dense gold foil flecks with occasional brighter facets, like the reference.
  for (let i = 0; i < 26000; i++) {
    ...
    ctx.fillStyle = facet > 0.985 ? "#fffaf0ee"
      : facet > 0.50 ? "#f1e3c19c" : "#78613d66";
    ctx.beginPath();
...
function startCelebrationEffects() {
  ...
  for (let i = 0; i < 36; i++) {
    ...
    particle.style.setProperty(
      "--particle-color",
      i % 3 === 0 ? "#dec391" : i % 3 === 1 ? "#b79a63" : "#fff5e8",
    );
